...
function load_locations!(locations::Vector{Location})
    for dir_file in readdir("public/locations")
        ...
        cover = false
        if isfile(full_path * "/images/cover.jpg")
            cover = true
        end
        descmd = div("null")
...
get_filepath(location::Location) = begin
    "locations/$(location.name)SEP$(location.position[1])SEP$(location.position[2])"
end
#==
extensions
==#
...
LOAD_LOCATIONS = Toolips.QuickExtension{:locs}()

function on_start(ext::Toolips.QuickExtension{:locs}, data::Dict{Symbol, Any}, rs::Vector{<:Toolips.AbstractHTTPRoute})
    load_locations!(ExploreTheCorners.locations)
end
...
menu_box = begin
    ...
    "archaeology" => "/images/icons/menu/arche.png", "contemporary pueblo" => "/images/icons/menu/contemporary.png", 
    "about" => "/images/icons/menu/about.png")
    mens = [begin 
        name, url = menpair[1], menpair[2]
        safename = replace(name, " " => "")
        new_men = div("$(safename)men", class = "menuitem", align = "center", 
        ...
        on(new_men, "click") do cl::ClientModifier
            style!(cl, "menubox", "opacity" => 0percent)
            next!(cl, "menubox") do cl2::ClientModifier
                redirect!(cl2, "/$name")
            end
        end
        new_men
    ...
    "display" => "grid", "animation-name" => "fadeup", "animation-duration" => 850ms, "z-index" => 19, "background" => "transparent", "transition" => 500ms)
    men_box
end
...
main = route("/") do c::Toolips.AbstractConnection
    ...
    main_svg = svg(width = 100percent, height = 100percent, text = read("public/images/maps/corners.svg", String))
...
# make sure to export!
export start!, main, default_404, logger, public_files, SESSION, LOAD_LOCATIONS
